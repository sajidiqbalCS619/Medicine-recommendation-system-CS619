...
SYNONYMS = {
    ...
    "sore throat": "throat_irritation", "swollen glands": "swelled_lymph_nodes",
    "swollen lymph nodes": "swelled_lymph_nodes", "cant smell": "loss_of_smell",
    "can not smell": "loss_of_smell", "tired": "fatigue",
...
def extract_symptoms(text, symptom_columns, fuzzy_threshold=0.85):
    ...
    # Avoid double-counting: "high fever" already implies fever, so don't
    # also add mild_fever from a bare "fever" mention in the same message.
    if "high_fever" in matched and matched.get("mild_fever") == "fever" and "mild fever" not in text_norm:
        del matched["mild_fever"]
...
page = st.sidebar.radio(
    "Navigate",
    ["AI Chatbot", "Dataset Preview", "Model Evaluation"],
)
    ...
    if user_text:
        ...
        with st.chat_message("assistant"):
            if not detected:
                ...
                if confidence < 45:
                    lines.append(
                        f"\n*Only {len(detected)} symptom(s) were detected, so this "
                        "prediction has lower confidence than usual — diseases in this "
                        "system are typically identified from several symptoms together "
                        "(3 or more). Try adding more detail (how long you've had it, "
                        "what else you're feeling) for a more reliable result.*"
                    )
                elif len(detected) == 1:
                    lines.append(
